...
**Multi-tenancy — motor conservado, modo "una institución por instalación" (ajuste posterior al Bloque 1):** El producto opera como **una sola institución por instalación**: cada cliente es una copia separada del software en su propio servidor. El **motor multi-institución** construido en el Bloque 0 (columna `institution_id` en cada tabla, `InstitutionScope`, trait `BelongsToInstitution`, `TenantAwareJob`, `TenantAwareCommand`, la suite de aislamiento) **se conserva intacto y DORMANTE** tras el flag `config('crm.multi_institution')` (por defecto `false`). Con el flag en `false` (modo normal): la institución activa se resuelve automáticamente a la única de la instalación, y se ocultan del panel las superficies multi-institución (cambiador de institución activa y gestión del atributo `is_super_admin`, que permanece en el esquema pero dormante). Con el flag en `true`, esas superficies reaparecen y el sistema se comporta multi-tenant. **Regla de esquema (siempre):** toda tabla de dominio nueva sigue llevando `institution_id` + el trait `BelongsToInstitution`, para mantener el motor coherente y la puerta abierta. La identidad de un contacto es única por `(institution_id + email)`. Si algún día se activa `multi_institution`, se hará una auditoría de aislamiento completa sobre todo el esquema en ese momento.

**Regla de pruebas de aislamiento (actualizada tras el Bloque 1):** la suite de aislamiento existente se **conserva y se re-ejecuta como regresión en cada bloque** (corre con el flag activo, verificando que el motor dormante sigue aislando). **Ya NO es obligatorio ampliarla** con casos de dos instituciones por cada bloque nuevo. Toda funcionalidad nueva sí necesita pruebas reales que la ejerciten; "verde" en PHPStan/Pint no basta.
